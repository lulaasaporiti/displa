...
export class ClienteSeleccionComponent implements OnInit {
  ...
  ngOnInit() {
    this.clienteService.getClientesVigentesList().subscribe(r => {
      this.clientes = r;
      ...
      .pipe(
        ...
        // map(Nombre => Nombre ? this._filter(Nombre) : this.clientes.slice())
      );
    });
  }
  ...
  setIdCliente(control, data) {
    if (control.value != null) data.idCliente = control.value.Id;
  }
